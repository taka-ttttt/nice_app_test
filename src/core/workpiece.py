...
class Workpiece:
    ...
    def create_material_mat024(self) -> kwd.Mat024:
        ...
        if self.material_properties is None:
            raise ValueError("材料特性が設定されていません。set_material_properties()を先に呼び出してください。")
    ...
    def create_material(self) -> Any:
        ...
        if self.material_type not in material_creators:
            ...
            raise ValueError(f"未対応の材料タイプ: '{self.material_type}'. 利用可能: {available}")
    ...
    def create_section_shell(
        ...
        if self.thickness is None:
            raise ValueError("板厚が設定されていません。set_thickness()を先に呼び出してください。")
