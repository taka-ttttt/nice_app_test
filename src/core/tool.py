...
class Tool:
    ...
    def set_following(
        ...
        following_config = FollowingConfig(
            leader_pid=leader_tool.id,
            threshold_displacement=threshold_displacement
        )
...
def create_punch(
    tool_id: int,
    name: str = "punch",
    material_constraint: str = "z-free"
) -> Tool:
    """パンチ工具を作成"""
    return Tool(
        id=tool_id,
