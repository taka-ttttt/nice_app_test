...
def parse_kfile(file_path: str) -> Tuple[List[ParsedPart], bool]:
    ...
    _process_element_dataframe(
        deck, "SHELL", part_data, part_names
    )
    
    # ELEMENT_SOLID を処理
    _process_element_dataframe(
        deck, "SOLID", part_data, part_names
    )
...
def _check_shared_nodes(part_data: Dict[int, Dict]) -> bool:
    ...
    for i, (pid1, data1) in enumerate(part_list):
        for pid2, data2 in part_list[i + 1:]:
