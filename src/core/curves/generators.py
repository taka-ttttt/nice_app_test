...
def create_stroke_curve(
    ...
    if stroke_mode not in valid_modes:
        ...
        raise ValueError(f"無効なストロークモード: '{stroke_mode}'. 利用可能: {available}")
...
def create_threshold_following_curve(
    ...
    if t_sw is None:
        raise ValueError(f"閾値変位量 {threshold_displacement} に到達しません。参照カーブの最大値: {max(y_ref)}")
