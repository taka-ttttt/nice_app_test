    ...
    with ui.card().classes('w-full bg-gray-50 p-3'):
        ...
        with ui.row().classes('w-full items-center justify-between mb-2'):
            ...
            delete_btn = ui.button(
                icon='delete',
                on_click=lambda: on_delete() if can_delete else ui.notify('最低1つの工具が必要です', type='warning'),
        ...
        with ui.row().classes('w-full gap-4 flex-wrap items-end'):
            ...
            ui.select(
                ...
                on_change=lambda e: setattr(tool, 'mesh_id', e.value if e.value else None),
            ).classes('w-56').props('dense')
            
            # 選択中のメッシュ詳細表示（折り畳み）
            if tool.mesh_id:
                selected_mesh = next((m for m in uploaded_meshes if m.id == tool.mesh_id), None)
                if selected_mesh:
                    with ui.expansion(
            ...
            ui.select(
                ...
                on_change=lambda e: (setattr(tool, 'motion_type', e.value), refresh_motion_details()),
            ).classes('w-28').props('dense')
        
        # 動作詳細行
        motion_detail_container = ui.row().classes('w-full gap-4 flex-wrap items-end mt-2')
        with motion_detail_container:
            render_motion_details()
