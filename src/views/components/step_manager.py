    ...
    def render_step_detail_content() -> None:
        ...
        with ui.row().classes('w-full items-center gap-4 mb-4'):
            ui.input(
                ...
                on_change=lambda e, s=current_step: (setattr(s, 'name', e.value), refresh_step_list()),
            ).classes('w-48')
        ...
        # 工具設定
        with ui.expansion('工具設定', icon='precision_manufacturing', value=True).classes('w-full'):
            ...
            with workpiece_container:
                for wp in step.workpieces:
                    render_workpiece_card(
                        ...
                        on_delete=lambda w=wp: (step.remove_workpiece(w.id), refresh_workpieces()),
            ...
            with splitter.before:
                with ui.column().classes('w-full h-full p-2 bg-gray-50'):
                    ...
                    with ui.row().classes('w-full gap-1'):
                        ui.button(icon='arrow_upward', on_click=move_step_up).props('flat dense').tooltip('上へ移動')
                        ui.button(icon='arrow_downward', on_click=move_step_down).props('flat dense').tooltip('下へ移動')
                    
                    # 複製・削除ボタン
                    with ui.row().classes('w-full gap-1'):
                        ui.button(icon='content_copy', on_click=duplicate_step).props('flat dense').tooltip('複製')
                        ui.button(icon='delete', on_click=delete_step).props('flat dense color=negative').tooltip('削除')
