            ...
            if state.friction.mode == FrictionMode.MANUAL:
                with ui.row().classes('gap-4 items-end'):
                    ui.number(
                        ...
                        step=0.01,
                        format='%.2f',
                        on_change=lambda e: setattr(state.friction, 'static_friction', e.value),
                    ...
                    ui.number(
                        ...
                        step=0.01,
                        format='%.2f',
                        on_change=lambda e: setattr(state.friction, 'dynamic_friction', e.value),
                    ).classes('w-28').props('dense')
...
def render_symmetry_settings(state: AnalysisConfig) -> None:
    ...
    with ui.column().classes('gap-2'):
        ...
        # 対称面使用チェックボックス
        use_symmetry_checkbox = ui.checkbox(
            '対称面を使用',
...
def render_constraint_settings(state: AnalysisConfig) -> None:
    ...
    def add_constraint() -> None:
        """拘束条件を追加"""
        constraint = ConstraintConfig.create(name=f'拘束条件 {len(state.constraints) + 1}')
    ...
    def render_constraint_item(constraint: ConstraintConfig) -> None:
        ...
        with ui.expansion(
            ...
            icon='lock',
            value=False,
        ).classes('w-full bg-gray-50').props('dense'):
            with ui.column().classes('gap-3 p-2'):
                ...
                with ui.grid(columns=4).classes('gap-2'):
                    ...
                    ui.number(
                        ...
                        step=1.0,
                        format='%.1f',
                        on_change=lambda e, c=constraint: setattr(c, 'x_range', (e.value, c.x_range[1])),
                    ...
                    ui.number(
                        ...
                        step=1.0,
                        format='%.1f',
                        on_change=lambda e, c=constraint: setattr(c, 'x_range', (c.x_range[0], e.value)),
                    ).classes('w-24').props('dense')
                    ...
                    ui.number(
                        ...
                        step=1.0,
                        format='%.1f',
                        on_change=lambda e, c=constraint: setattr(c, 'y_range', (e.value, c.y_range[1])),
                    ).classes('w-24').props('dense')
                    ui.label('~').classes('text-sm self-center')
                    ui.number(
                        label='max',
                        ...
                        step=1.0,
                        format='%.1f',
                        on_change=lambda e, c=constraint: setattr(c, 'y_range', (c.y_range[0], e.value)),
                    ).classes('w-24').props('dense')
                    ...
                    ui.number(
                        ...
                        step=1.0,
                        format='%.1f',
                        on_change=lambda e, c=constraint: setattr(c, 'z_range', (e.value, c.z_range[1])),
                    ).classes('w-24').props('dense')
                    ui.label('~').classes('text-sm self-center')
                    ui.number(
                        label='max',
                        ...
                        step=1.0,
                        format='%.1f',
                        on_change=lambda e, c=constraint: setattr(c, 'z_range', (c.z_range[0], e.value)),
                    ).classes('w-24').props('dense')
                ...
                with ui.row().classes('gap-4'):
                    ...
                    with ui.column().classes('gap-1'):
                        ui.checkbox(
                            ...
                            on_change=lambda e, c=constraint, i=0: update_dof(c, i, e.value),
                        )
                        ui.checkbox(
                            'Y並進',
                            value=constraint.dof[1],
                            on_change=lambda e, c=constraint, i=1: update_dof(c, i, e.value),
                        )
                        ui.checkbox(
                            'Z並進',
                            value=constraint.dof[2],
                            on_change=lambda e, c=constraint, i=2: update_dof(c, i, e.value),
                        )
                    ...
                    with ui.column().classes('gap-1'):
                        ui.checkbox(
                            ...
                            on_change=lambda e, c=constraint, i=3: update_dof(c, i, e.value),
                        )
                        ui.checkbox(
                            'Y回転',
                            value=constraint.dof[4],
                            on_change=lambda e, c=constraint, i=4: update_dof(c, i, e.value),
                        )
                        ui.checkbox(
                            'Z回転',
                            value=constraint.dof[5],
                            on_change=lambda e, c=constraint, i=5: update_dof(c, i, e.value),
                        )
